...
async function showScreen(screen) {
  ...
  const SCREEN_TITLES = {
    ...
    ops: 'Операционная сводка', warehouse: 'Склад',
  };
...
//
// Локальные таблицы, МойСклад здесь не участвует. Три под-вкладки — ровно
// три экрана из ТЗ.
// ═══════════════════════════════════════════════════════════════════════════

let whTab = 'stock';            // 'stock' | 'invoices' | 'new'
...
function whEmpty(ic, title, hint) {
  return `<div class="empty-state">
    <div class="empty-state-icon">${icon(ic)}</div>
    <div class="empty-state-title">${escapeHtml(title)}</div>
    <div class="empty-state-hint">${escapeHtml(hint)}</div>
  </div>`;
}

async function renderWarehouseScreen() {
  const content = document.getElementById('content');
  content.innerHTML = '<div class="spinner-wrap"><div class="spinner"></div></div>';

  const TABS = [
    ['stock', 'Остатки', 'box'],
    ['invoices', 'Накладные', 'list'],
    ['new', 'Новая', 'plus'],
  ];
  const tabsHtml = `
    <div class="seg-row">
      <div class="seg">
        ${TABS.map(([id, label, ic]) => `
          <button class="seg-item ${whTab === id ? 'active' : ''}" data-wh="${id}"
                  aria-pressed="${whTab === id}">${icon(ic)} ${label}</button>`).join('')}
      </div>
    </div>`;

  try {
    if (whTab === 'stock') await renderWhStock();
    else if (whTab === 'invoices') await renderWhInvoices();
    else await renderWhInvoiceNew();
  } catch (e) {
    content.innerHTML = errorBox(e.message || String(e));
  }

  // Переключатель поверх вывода — как в «Заказах» (renderOrdersScreen).
  document.getElementById('content').insertAdjacentHTML('afterbegin', tabsHtml);
  document.querySelectorAll('.seg-item[data-wh]').forEach(btn => {
    btn.addEventListener('click', () => {
      haptic('light');
      whTab = btn.dataset.wh;
...
async function renderWhStock() {
  const content = document.getElementById('content');
  const data = await api('/api/wh/stock', {});
  whStockCache = data.products || [];

  if (!whStockCache.length) {
    content.innerHTML = whEmpty('box', 'Номенклатура пуста',
      'Перенесите справочник из МойСклад: python -m scripts.migrate_from_moysklad --apply');
    return;
  }
...
function drawWhStockList() {
  ...
  list.innerHTML = rows.length
    ...
    : whEmpty('search', 'Ничего не найдено', 'Измените поисковый запрос');
}

// ─── Экран 2: список накладных ───────────────────────────────────────────────

async function renderWhInvoices() {
  const content = document.getElementById('content');
  const data = await api('/api/wh/invoices', { limit: 100 });
  const rows = data.invoices || [];

  if (!rows.length) {
    content.innerHTML = whEmpty('list', 'Накладных пока нет',
      'Оформите первую на вкладке «Новая»');
    return;
  }

  const canCancel = whIsBoss();
  content.innerHTML = rows.map(inv => {
...
// ─── Экран 3: быстрый ввод накладной ─────────────────────────────────────────
...
async function renderWhInvoiceNew() {
  ...
  if (!products.length) {
    content.innerHTML = whEmpty('box', 'Нет номенклатуры',
      'Сначала перенесите товары из МойСклад скриптом миграции');
    return;
  }
