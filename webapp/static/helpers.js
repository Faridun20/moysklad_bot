...
(function (root, factory) {
  ...
  function stockTabs(f) {
    ...
    // «Движения» — всё, отчего остаток меняется: приход, отгрузки, списания,
    // перемещения. Вкладка, а не кнопка между табами и поиском: у неё свои
    // действия. «Залежалось» стало фильтром каталога (только руководству —
    // ручка /api/channel/stale отвечает admin/boss): это срез того же списка
    // товаров, а не другой экран. `canInvoices: false` — руководитель без
    // «Рабочих действий»: движения склада это работа.
    //
    // Называлась вкладка «Накладные», а внутри снова лежали «накладные» —
    // одно слово на двух уровнях, и человек не понимал, куда он попал
    // (жалоба владельца). Имя выбрал он сам. КЛЮЧ при этом остался
    // `invoices`: на него завязаны адреса `stock:invoices`, алиас
    // `whinvoices` в LEGACY_SCREENS, ссылки из очереди дел и закладки —
    // переименовать ключ значит сломать их ради слова на кнопке.
    if (f.canSeeGoods && f.canInvoices !== false) tabs.push({ key: 'invoices', label: 'Движения' });
    return tabs;
  }
  ...
  function moneyTabs(f) {
    ...
    // «Склад → Движения → Списания». Право режется там же, ручкой:
