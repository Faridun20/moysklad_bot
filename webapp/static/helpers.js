...
(function (root, factory) {
  ...
  function moneyTabs(f) {
    ...
    // «Поставщикам» — зеркало «Долгов»: там нам должны, здесь должны мы.
    // Отдельной вкладкой, а не переключателем внутри «Долгов»: суммы там
    // считаются по заказам клиентов, здесь — по приходным накладным, и один
    // экран на два разных расчёта читался бы как один общий итог. Только
    // руководству: сумма прихода это закупочная цена (`/api/suppliers/debts`
    // отвечает admin/boss), и вкладка, которая вернёт 403, хуже отсутствующей.
    if (f.canSeeSuppliers) tabs.push({ key: 'suppliers', label: 'Поставщикам' });
  ...
  function roleSectionTabs(section, role, opts) {
    ...
    if (section === 'money') {
      return moneyTabs({
        ...
        canSeeDebts: working,
        // «Поставщикам» — руководству всегда: это контроль, а не работа
        // менеджера, и за выключатель «Рабочие действия» она не прячется.
        canSeeSuppliers: boss,
