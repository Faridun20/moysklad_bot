...
**Шаблоны расписки и тилхата собирает скрипт** (`scripts/build_legal_templates.py`
→ `templates/legal/*.docx`, прогон `python -m scripts.build_legal_templates`).
.docx двоичный: правку в нём не видно в диффе, не отревьюить и не сделать без
Word, которого на сервере нет. Исходник — скрипт, файл в репозитории — результат
его прогона; расхождение ловит
`tests/test_legal_docs.py::test_committed_templates_match_the_generator`
(сравниваются тексты абзацев, не байты: zip несёт таймстемпы). Вид взят с
образца руководства: «Расписка» по центру обычным начертанием, город и дата
ОДНОЙ строкой (правая табуляция, не два абзаца по краям), тело Times New Roman
14 пт по ширине с абзацным отступом, сумма отдельной жирной строкой, согласие
должника жирным, подпись — линейка во всю ширину (нижняя граница абзаца, а НЕ
строка подчёркиваний: её ширина зависит от шрифта и переноса) с мелкой подписью
под ней. Юридическое содержание при этом то же: преамбула лишь собрала в себя
прежние пункты 1–2 (кто у кого что получил и за сколько), как в образце.
Приметы вида держит `test_template_keeps_the_look_of_the_sample`.

**Порядок оплаты в документе выводится из ЧИСЛА платежей.** Отдельного
переключателя «Разовый / Рассрочка» нет: два поля противоречили друг другу —
менеджер вписывал шесть платежей, забывал переключить «Разовый», и расписка
молча выходила с ОДНОЙ строкой графика на всю сумму и остатком 0. Источник
истины один: `installments_count` ≥ 2 — рассрочка с графиком, 1 или пусто —
разовый платёж. Явно заказанная через API рассрочка с одним платежом — ошибка
формы, а не тихий разовый платёж.
