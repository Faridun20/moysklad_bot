...
один пересобрался, второй остался на прежнем коммите, и «половина починилась»
выглядит как новый баг. Сверить их можно, только пока оба считают SHA
одинаково, поэтому `webapp/server.py` больше не считает его у себя.
- Источник — переменные платформы (`RAILWAY_GIT_COMMIT_SHA`,
  `RAILWAY_GIT_COMMIT_MESSAGE`), а НЕ `.git`: Railpack кладёт исходники без
  истории, и в контейнере репозитория нет. `git rev-parse` — второй шаг для
  локального запуска, таймстамп старта — третий (он не отвечает «какой
  коммит», но отвечает «когда перезапустили»).
