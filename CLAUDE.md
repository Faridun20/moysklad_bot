...
инкрементальных миграций нет). Товары сопоставляются ТОЧНО по нормализованному
названию; похожие имена не склеиваем — угадывание означает приход не на ту
карточку, а несопоставленное возвращается списком «заведите товар», потому что
молча пропущенная позиция это остаток, которого нет. `sync_supply` не создаёт,
а СИНХРОНИЗИРУЕТ: количества правятся сутки, и документ едет за ними (POST →
PUT по `ms_supply_id`).
