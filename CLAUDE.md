...
само число наружу не идёт. «Залежалось» — вкладка в «Складе» (только
руководству, как и ручка): экран ВНУТРЕННИЙ, поэтому остаток там виден — по нему
и решают, что выносить, — а в пост из отмеченного уходят одни названия. `channel_posts` хранит историю, чтобы один контейнер
