...
собеседника ни в каком поле апдейта. Он живёт на карточке контрагента МойСклад,
...
(`services/ms_counterparty.create_counterparty`). Заводит ЧЕЛОВЕК кнопкой:
каждый написавший — ещё не клиент, автосоздание превратило бы справочник в
свалку. Дубликат ловим по снапшоту, свежесозданного кладём туда сразу
(`snapshot.remember_counterparty`), иначе до ночной синхронизации того же
клиента заведут второй раз. Поиск контрагента идёт и по названию, и по ТЕЛЕФОНУ
(цифры без разделителей — в МС номер лежит свободным текстом).
