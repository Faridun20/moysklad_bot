...
def test_partial_return_sets_partially_returned(shipped_order):
    ...
    item_a = items[0]
    r = db.create_return(oid, "partial", "брак", [(item_a["id"], 1, 100.0)],
                         refund_method="debt_reduction", created_by=mgr)
    assert r["ok"] is True
...
def test_cash_refund_creates_negative_deposit(shipped_order):
    db, mgr, oid, items = shipped_order
    r = db.create_return(oid, "partial", "брак", [(items[1]["id"], 1, 50.0)],
                         refund_method="cash", created_by=mgr)
...
def test_return_confirm_is_idempotent(shipped_order):
    db, mgr, oid, items = shipped_order
    r = db.create_return(oid, "partial", "x", [(items[0]["id"], 1, 100.0)],
...
def test_return_blocked_for_draft(isolated_db):
    ...
    items = db.get_order_items(oid)
    r = db.create_return(oid, "partial", "x", [(items[0]["id"], 1, 10.0)],
                         refund_method="no_refund", created_by=mgr)
...
def test_confirmed_return_reduces_agent_debt(shipped_order):
    ...
    r = db.create_return(oid, "partial", "брак", [(items[0]["id"], 1, 100.0)],
                         refund_method="debt_reduction", created_by=mgr)
    db.confirm_return(r["return_id"], 1, "Boss")
