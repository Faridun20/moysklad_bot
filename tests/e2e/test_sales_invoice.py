...
def test_manager_shows_the_invoice_before_shipping_and_then_ships(open_app, e2e):
    ...
    tab(mgr, "orders")
    settled(mgr)
    # «Счёт на оплату» — в подробной сводке карточки. Её раскрывали выше, а
    # состояние «Подробнее» переживает перерисовку списка: жмём, только если
    # сводка свёрнута.
    if mgr.locator("#content [data-details-toggle]").first.get_attribute("aria-expanded") != "true":
        mgr.click("#content [data-details-toggle]")
    mgr.wait_for_selector(".btn-sales-invoice")
