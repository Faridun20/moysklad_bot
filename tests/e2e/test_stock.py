...
def test_tab_row_shows_that_it_scrolls(open_app, e2e):
    ...
    У босса в «Складе» четыре вкладки. Вкладки берут ширину по подписям и на
    390dp влезают целиком, а на узком телефоне (340dp) последняя («Движения»)
    не влезает и обрывается ровно по краю — без затенения это читается как «так
    и задумано». Признак прокрутки ставит refreshScrollHints по ФАКТУ
    переполнения, поэтому проверяем наблюдаемое: атрибут у ряда и то, что он
    снимается, когда ряд долистали до конца.
    """
    boss = open_app(e2e.ids["boss"])
    boss.set_viewport_size({"width": 340, "height": 740})
    go(boss, "stock")
...
def _open_writeoffs(page) -> None:
    """«Склад → Движения → Списания». Пятой вкладкой раздела это быть не может
