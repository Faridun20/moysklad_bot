...
def _table_ddls() -> list[str]:
    ...
    tables = [
            ...
            # Рейт-лимит прямого напоминания клиенту-должнику в Telegram (B3,
            # `services/client_debt_reminders.py`). Долг открыт день за днём, и
            # напоминание хочется слать КАЖДЫЙ день, пока он не закрыт — но не
            # дважды за один прогон/сутки. PRIMARY KEY по order_id, а не
            # UNIQUE(order_id, date): нужна только ПОСЛЕДНЯЯ отметка, история
            # рассылок не нужна — она и так есть в audit_log.
            """CREATE TABLE IF NOT EXISTS client_debt_reminders (
                order_id       BIGINT PRIMARY KEY,
...
_DEFAULT_SETTINGS: dict[str, tuple] = {
    ...
    # B3: прямое напоминание клиенту-должнику в Telegram (при наличии
    # telegram_id у контрагента). По умолчанию ВЫКЛ — рассылка живому клиенту
    # требует явного решения владельца; включает руководство в «Настройках».
    "client_debt_reminders_enabled": (False, "Напоминания о долге — напрямую клиенту в Telegram"),
