...
**Сделки по технике.** Одобрение сделки по технике — как заявка на
отгрузку: решение блокирует продажу, поэтому оно ВСЕГДА немедленное и
порогом не режется. Карточку шлёт `services.machine_deal_requests.
notify_decision_card` и спрашивает `should_notify_now(MACHINE_DEAL_APPROVAL)`
перед отправкой — станет правило условным, менять придётся только здесь.
"""
