...
Что порог МЕНЯЕТ в потоке (важно для понимания C5): новой «нельзя отгрузить»
не появляется — её и не должно быть. Каждый заказ и так проходит одобрение
руководителя (`draft → pending → approved`, `order_workflow.submit_order`,
одобряют только admin/boss), поэтому второй блокирующий шлагбаум лишь спорил
бы с уже существующим `payment_required` из `services.order_payments`. Порог
делает две вещи:
1. помечает заявку в очереди решений (бот-карточка и «Решения» в WebApp) —
   видно, ГДЕ уступили и сколько;
2. требует от одобряющего ЯВНОГО подтверждения — как превышение кредитного
   лимита: `order_workflow.approve_shipment_request` отвечает
   `needs_discount_ack`, и одобрение проходит только вторым нажатием
   (`discount_ack=True`), а факт уходит в аудит.
Менеджеру при этом показывается обычная строка состояния — `pending_note`,
в стиле отказов `payment_required`/`stock_not_written_off`.
"""
