...
def build_reminder_text(order_id: int, amount_cents: int, currency: str, due_date: str) -> str:
    ...
    lines = [
        "Здравствуйте!",
        "",
        f"Напоминаем, что по заказу №{int(order_id)} остаётся долг:",
    ...
    lines += [
        "",
        "Пожалуйста, свяжитесь с нами, чтобы закрыть долг. Если вы уже оплатили — "
        "извините за беспокойство, просто не отвечайте на это сообщение.",
