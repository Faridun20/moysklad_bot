...
SCREEN_MAP = [
    ...
    # «Клиенты» — список ПОКУПАТЕЛЕЙ (сколько купил / сколько должен / когда
    # отгружали): менеджеру тоже, как и карточка клиента (A3). «Лимиты» —
    # вторая вкладка того же раздела, она остаётся руководству.
    ...
    ("Заказы → «Отгрузить» / «Внести оплату и отгрузить» — без одобрения (руководство — с «Рабочими действиями»)", "admin, boss, manager, warehouse_keeper", "/api/orders/ship"),
    ("Черновик → заявка на отгрузку руководителю (только скидка выше порога / долг сверх лимита)", "admin, boss, manager", "/api/orders/submit"),
    ...
    ("Оплата → «Куда поступили»; Настройки → Карты и счета", "admin, boss, manager (правка и архив — руководство)",
     "/api/pay_accounts"),
    ("Деньги → Сверка кассы", "admin, boss, manager (чужую историю видит только руководство)",
     "/api/cash/reconcile"),
]
