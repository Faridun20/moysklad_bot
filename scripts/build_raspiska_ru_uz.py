...
**Подписанта кредитора в бланке нет** (решение владельца, 2026-09). Расписка —
односторонний документ ДОЛЖНИКА (физлица): её пишет и подписывает он, а
компании между собой обмениваются договорами. Поэтому из бланка убраны
«в лице [должность, Ф.И.О. представителя], действующего на основании
[Устава / доверенности …]» (и узбекское «номидан … асосида иш юритувчи») и
строки кредитора «В лице / (подпись) / Должность … / М.П.» (узб. «Номидан /
(имзо) / Лавозими … / М.Ў. (муҳр ўрни)»). Кредитор опознаётся названием, ИНН
и юридическим адресом. Правка сделана В ИСХОДНИКЕ (его и показывать юристу),
а `FORBIDDEN_PHRASES` не даёт вернуть её молча старым бланком.
...
# Фразы подписанта кредитора, которых в расписке быть не должно (см. докстринг):
# вернулись в исходнике — значит, в Word открыли старый бланк юриста, и сборка
# обязана остановиться, а не снова просить у менеджера «должность подписанта».
FORBIDDEN_PHRASES = (
    "в лице", "действующего на основании", "Должность:", "М.П.",
    "номидан", "асосида иш юритувчи", "Лавозими:", "М.Ў.",
)
...
def build(variant: str = "raspiska_ru_uz") -> Document:
    ...
    source_text = _body_text(doc)
    back = [phrase for phrase in FORBIDDEN_PHRASES if phrase in source_text]
    if back:
        raise SystemExit(f"в бланке снова подписант кредитора: {back} — расписку пишет должник")
