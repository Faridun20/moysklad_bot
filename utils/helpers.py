...
def local_now() -> datetime:
    """Текущее локальное время — ТЕМ ЖЕ способом, что пишется `created_at`
    (`now_str()` = `datetime.now()`, локальная TZ контейнера).

    WP-18: раньше было `utc_now() + TZ_OFFSET`. При `container TZ != TZ_OFFSET`
