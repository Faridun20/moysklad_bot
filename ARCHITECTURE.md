...
заказы, фиксируют оплаты и сами отгружают товар (одобрение отгрузки не
обязательно — руководителю приходит уведомление «Заказ отгружен»; на его
решение уходят только заказы со скидкой выше порога или долгом сверх лимита),
руководители подтверждают поступление денег.
...
- **Заявка на отгрузку** (`shipment_requests`) — заказ, отправленный
  руководителю на решение (скидка выше порога, долг сверх лимита). При
  отгрузке без одобрения заявка тоже заводится и проводится тем же
  переходом — её одобряет сам отгружающий (`approved_by` = автор).
...
| `boss` | Решает по заявкам на отгрузку со скидкой выше порога или долгом сверх лимита (остальные менеджер отгружает сам), получает уведомление о каждой отгрузке, подтверждает поступление денег по заказам, видит компанию в Аналитике, видит все долги, видит журнал действий (`/api/audit_log`, «Настройки → Журнал действий» — тот же аудит, что у admin в боте `/audit`). Получает уведомления о новых отгрузках и платежах. |
| `manager` | Создаёт заказы и сам их отгружает («Отгрузить»; заявка руководителю — только при скидке выше порога или долге сверх лимита), отправляет платежи на подтверждение, видит свои заказы и свои долги. Отмечает «деньги получил» — но не закрывает долг сам. |
...
Когда заявку одобряют, `approve_shipment_request` одной транзакцией двигает
заявку и заказ в `approved`, а `services/order_shipment.py` проводит расходную
накладную — она и списывает остаток. Одобряет руководитель (заявка со скидкой
выше порога / долгом сверх лимита) либо сама отгрузка без одобрения
(`order_workflow.ship_order_now`): тогда `approved_by` — автор заказа, в
журнале `shipment_auto_approved`.
...
order: status=draft, agent, items, payment_type, due_date?
   ...
   │
   │ нажал «Отгрузить» / «Внести оплату и отгрузить»
   │ (order_workflow.ship_order_now; одобрение отгрузки НЕ обязательно)
   │
   ├─ проверки ДО любого перехода: клиент, позиции связаны с каталогом,
   │  остаток на складе (order_shipment.preflight), условия оплаты, у «оплаты
   │  сразу» — разбивка сходится с суммой (order_payments.check_payment_parts).
   │  Отказ — черновик остаётся черновиком.
   │
   ├─ скидка ≥ порога или долг сверх лимита → отказ code=decision_required;
   │  менеджер отправляет заявку (/api/orders/submit) — ветка ниже
   │
   ├─ иначе одной операцией (каждый шаг — своя транзакция, операция
   │  возобновляема: прервалась — повторное «Отгрузить» продолжит):
   │     submit_order → approve_shipment_request(without_approval) →
   │     накладная, остаток списан → разбивка оплаты → mark_order_shipped
   │     order: status=shipped
   │     push (фоном) → все boss/admin: «Заказ #N отгружен» — клиент, кто
   │     отгрузил, все позиции, итог, как оплачено / в долг до; без кнопок
   │
   │ заявка (только скидка / лимит)
   ▼
...
push → все boss/admin
   ...
   │     (отгружает менеджер сам, руководителю — push «Заказ отгружен»)
   │     order: status=shipped
...
Во фронте всё это — вкладка «Склад → Движения» (ключ адреса `stock:invoices`)
с четырьмя видами вторым уровнем: **Приход · Отгрузки · Списания ·
Перемещения**. Вкладка называлась «Накладные», и внутри снова лежали
«накладные» — одно слово на двух уровнях (жалоба владельца). Ключ адреса при
переименовании не менялся: на нём висят `LEGACY_SCREENS`, ссылки очереди дел
и закладки.
...
одобрение (два босса, ретрай, старая кнопка, двойное «Отгрузить») не спишет
товар дважды. При отгрузке без одобрения `order_shipment.preflight` заранее
проверяет позиции и остаток — пока заказ ещё черновик и его можно поправить;
решает по-прежнему `ship_order` под замком склада.
...
- `handlers/orders.py` — решения по заявке (одобрить / отклонить / на доработку /
  ...
  Кнопка решения на карточке заявки, которую менеджер уже отгрузил сам,
  гаснет с исходом «🚚 Менеджер отгрузил сам».
- `handlers/order_ship.py` — `/ship НОМЕР`: тот же `ship_order_now`, что у кнопки
  «Отгрузить»; «оплату сразу» отправляет в WebApp.
...
| `/api/orders/requests` | POST | заявки, которые ждут решения руководителя — скидка выше порога или долг сверх лимита (только boss) |
...
| `/api/orders/submit` | POST | заявка руководителю, когда без его решения не отгрузить (тут принимаются payment_type + due_date) |
| `/api/orders/ship` | POST | «Отгрузить»: черновик / заявка без решения / одобренный заказ; `payment_type`, `due_date`, `parts` (разбивка «оплаты сразу»); отказы с `code`: `payment_required`, `decision_required`, `insufficient_stock`, … |
