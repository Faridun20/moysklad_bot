...
| Round 7 (HTML-инъекция: agent_name) | `handlers/orders.py` «Клиент выбран» интерполировал имя контрагента из МойСклад в `parse_mode="HTML"` без `esc()` — имя с `<`/`&` ломало отправку / инжектило разметку. Единственное необёрнутое место (свип). Фикс `_esc(agent['name'])` | PR #92 |
| Round 7 (энфорс кредит-лимитов) | `approve_shipment_request` для credit-заявки считает `check_credit_limit`; превышение требует явного override боссом (`req_ovr:` / `override=true`) → `orders.credit_limit_override` + audit. Раньше `over_limit` был чисто информационным | PR #93 |
| Round 7 (деактивация = потеря прав) | `get_role` отдаёт `guest` если `user_roles.deactivated_at` стоит → деактивированный теряет ВСЕ права (auth/_authorize), исключается из notify-recipients. `/deactivate` (admin) + webapp `/api/users/deactivate` | PR #94 |
| Round 7 (reject_payment idempotency) | `/api/orders/reject_payment` делал только `_idem_get` без `_idem_set` → ретрай с тем же ключом слал дубль-уведомление об отклонении. Добавлен `_idem_set` | PR #97 |
| Round 7 (архив аудита — потеря данных) | `run_maintenance` чистил `audit_log` даже если экспорт в архив упал → удаление неархивированного. Теперь `prune_audit_log` только при `ok` экспорта; `get_audit_log_before` с `LIMIT` (anti-OOM) | PR #97 |
...
| 2026-05-31 | Раунд 7 (большая фича-сессия + 2 скана): HTML-инъекция agent_name, энфорс кредит-лимитов + override, деактивация=guest, reject_payment idempotency, фикс потери данных в архиве аудита, синхронизация MS-delete (вебхук + cron), перф (индексы + N+1 fix + to_thread), backup version-mismatch fallback. PR #92–#98 | Claude Opus 4.8 |
